...
class Residents extends Component
{
    ...
    {
        $families = FamilyHead::where('zone', auth()->guard('bhw')->user()->assigned_zone)
        ...
        $heads = FamilyHead::with('familyMembers')->where('zone', auth()->guard('bhw')->user()->assigned_zone)->get();
        $wife = Wife::where('zone', auth()->guard('bhw')->user()->assigned_zone)->count();
        $members = 0;
        
        foreach($heads as $head){
            $members += $head->familyMembers->count();
        }

        $residents = $heads->count() + $wife + $members;

        return view('livewire.sub-b-h-w.residents', [
            'families' => $families,
            'residents' => $residents,
            'total_family' => $heads->count(),
        ]);
    }
}
