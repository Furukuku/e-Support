<div class="bg-white officials-profile-table shadow rounded pt-3 mt-2 mb-5">
  @include('livewire.modals.declined-brgy-clearances-modals')
      ...
      <tbody>
        @forelse ($declined_documents as $declined_doc)
          <tr>
            ...
            <td class="align-middle text-center">{{ $declined_doc->brgyClearance->name }}</td>
            <td class="align-middle text-center">{{ $declined_doc->brgyClearance->purpose }}</td>
            <td class="align-middle text-center">{{ $declined_doc->updated_at->format('h:i A - M d, Y') }}</td>
            <td class="align-middle text-center">
              <i class="fa-solid fa-eye mx-1 align-middle view-icon" wire:click="view({{ $declined_doc }})" data-bs-toggle="modal" data-bs-target="#declinedBrgyClearanceInfo"></i>
            </td>
          </tr>
        @empty
          <tr>
