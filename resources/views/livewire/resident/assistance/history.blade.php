      ...
      <div wire:poll.60s class="col-auto mb-3">
        <div class="card shadow-sm requested-docs-card">
          <div class="card-body pt-4 pb-5 rounded-top even-requested-docs-card-body">
            <h5 class="card-title text-white">{{ $myAssistanceHistory->purpose }}</h5>
          ...
          <div class="card-footer d-flex justify-content-between bg-white">
            <div class="d-flex">
              ...
              <div class="rounded-pill {{ $myAssistanceHistory->status === 'Rejected' ? 'bg-danger' : 'bg-success' }} text-white ms-2 px-3">
                {{ $myAssistanceHistory->status }}
              </div>
            </div>
            @if ($myAssistanceHistory->status === 'Rejected')
              <div>
  ...
  <script>
    document.getElementById('comment').addEventListener('click', () => {
      Swal.fire({
        ...
        text: "{{ $myAssistanceHistory->comment }}",
        icon: "question",
